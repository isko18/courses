...
- `ADMIN_VIDEO_MAX_MB = 200` — макс. размер файла при загрузке через админку (МБ); большие — через API
...
**Чтобы сайт не зависал:** запускайте приложение с несколькими воркерами (`gunicorn -w 4`). В админке разрешена загрузка видео до `ADMIN_VIDEO_MAX_MB` (по умолчанию 200 МБ); большие файлы — через API. При загрузке через API обработка идёт **асинхронно** (ответ 201 сразу, статус «ready»/«error» — по опросу урока).
...
| Метод | URL | Описание |
|-------|-----|----------|
| POST | `/api/teacher/lessons/create-with-upload/` | Загрузка урока с видео (FormData). Обработка (сжатие, сохранение) выполняется **асинхронно**; ответ 201 с `youtube_status: "processing"`. Готовность — по полю `youtube_status` (`ready` / `error`). |
...
- **Редактирование урока:** можно загружать видео через поле «Video file». В админке действует лимит размера (`ADMIN_VIDEO_MAX_MB`, по умолчанию 200 МБ); файлы больше — только через API. Чтобы сайт не зависал при загрузке, запускайте приложение с несколькими воркерами: `gunicorn -w 4`.
- Просмотр: превью в форме урока или кнопка «▶ Просмотр» в новой вкладке.
