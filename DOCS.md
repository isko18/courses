...
**Чтобы сайт не зависал:** запускайте приложение с несколькими воркерами (`gunicorn -w 4`). В админке можно загружать видео любого размера (лимит — общий `VIDEO_MAX_SIZE_GB`). При загрузке через API обработка идёт **асинхронно** (ответ 201 сразу, статус «ready»/«error» — по опросу урока).
...
- **Редактирование урока:** можно загружать видео через поле «Video file» (лимит — общий, до 20GB). Чтобы сайт не зависал при загрузке больших файлов, запускайте приложение с несколькими воркерами: `gunicorn -w 4`.
