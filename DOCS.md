...
**Чтобы сайт не зависал при загрузке видео:** загрузка через админку отключена (файл только для чтения). Используйте API: `POST /api/teacher/lessons/create-with-upload/`. На продакшене запускайте приложение несколькими воркерами, например: `gunicorn -w 4 ...`, чтобы один долгий запрос не блокировал весь сайт.
...
- **Редактирование урока:** поле «Video file» только для чтения (загрузка через админку отключена, чтобы не блокировать сайт). Можно указать ссылку на видео (video_url). Для загрузки файла используется API: `POST /api/teacher/lessons/create-with-upload/`.
